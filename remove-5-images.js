...
const updated = MACHINERY_DATA.map((item) => {
  if (item.images) {
    const filteredImages = item.images.filter(img => !urlsToRemove.has(img));
    return {
      ...item,
      images: filteredImages.length > 0 ? filteredImages : item.images
    };
  }
  return item;
});
...
console.log('Successfully removed requested 5 images from all products in catalog!');
